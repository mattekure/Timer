...
function startTimerLoop(url, response)
	local tJSON = Utility.decodeJSON(response)
	nTimerStartTime = tJSON.unixtime
	nCurrentTime = tJSON.unixtime
	loopTimer()
end
...
function loopTimer(url, response)
	if timerRunning then
		...
		Interface.openURL("https://worldtimeapi.org/api/timezone/Etc/UTC", loopTimer)	
	end
end
